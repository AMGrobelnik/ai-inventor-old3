...
// State management
let currentGraphType = 'concepts';
let currentTopic = 'all';
let currentRelation = 'all';
let ontologyLayout = 'force';
let ontologyFilter = 'overlap';
let crossTopicLayout = 'fixed';
let showGaps = true;
let showBestOnly = true;
let availableYears = [];
let availableTopics = [];
...
let isUpdatingTopicFilter = false;
const PLAY_INTERVAL_MS = 5000;
...
function updateLegend(graphType) {
    ...
    if (legendEl)
        legendEl.innerHTML = legend.html;
}
/**
 * Discover available graph files
 */
async function discoverGraphFiles() {
    const baseFiles = [
        { value: 'all', label: 'All Years', file: 'data/cooccurrence/all.json' }
    ];
    const yearPromises = [];
    for (let year = 2015; year <= 2025; year++) {
        yearPromises.push(fetch(`data/cooccurrence/by_year/${year}.json`)
            .then(response => {
            if (response.ok) {
    ...
    const validYears = yearResults.filter((y) => y !== null);
...
function extractTopics(data) {
    ...
    data.nodes.forEach(node => {
        ...
        if (Array.isArray(nodeTopics)) {
            nodeTopics.forEach((t) => topics.add(t));
        }
    });
...
function filterByTopic(data, topic) {
    if (topic === 'all')
        return data;
    const topicNodeIds = new Set(data.nodes.filter(node => {
        if (node.topic === topic)
            return true;
        if (Array.isArray(node.topics) && node.topics.includes(topic))
            return true;
        return false;
    }).map(n => n.id));
 ...
 */
function filterByRelation(data, relation) {
    if (relation === 'all')
        return data;
 ...
 */
function filterOntologyOverlap(data) {
    const multiTopicNodeIds = new Set(data.nodes.filter(node => {
        const nodeTopics = node.topics;
        return Array.isArray(nodeTopics) && nodeTopics.length > 1;
    }).map(n => n.id));
    if (multiTopicNodeIds.size === 0)
        return data;
 ...
 */
function filterCrossTopicGaps(data, selectedTopic) {
    ...
    const filteredNodes = data.nodes.filter(node => node.id === selectedTopicNode.id || gapConceptIds.has(node.id));
    return { nodes: filteredNodes, edges: gapEdges };
}
/**
 * Filter blind spots to only include top N% of concepts by score
 */
function filterBlindSpotsTop(data, topPercent = 0.05) {
 ...
 */
function setupYearSliderFromData(years) {
    ...
    return {
        ...
        formatter: function (params) {
            if (params.dataType === 'node') {
                ...
                if (d.type)
                    html += `<br/>Type: <span style="color:#94a3b8">${d.type}</span>`;
                if (d.topic)
                    html += `<br/>Topic: <span style="color:#94a3b8">${d.topic}</span>`;
                if (d.count !== undefined)
                    html += `<br/>Count: <span style="color:#3b82f6">${d.count}</span>`;
                if (d.citations !== undefined)
                    html += `<br/>Citations: <span style="color:#34d399">${d.citations.toLocaleString()}</span>`;
                if (d.total_citations !== undefined)
                    html += `<br/>Citations: <span style="color:#34d399">${d.total_citations.toLocaleString()}</span>`;
                if (d.pagerank !== undefined)
                    html += `<br/>PageRank: <span style="color:#fbbf24">${d.pagerank.toFixed(2)}</span>`;
                if (d.normalized_score !== undefined)
                    html += `<br/>Score: <span style="color:#f59e0b">${(d.normalized_score * 100).toFixed(0)}%</span>`;
                return html;
            }
                ...
                if (d.count !== undefined)
                    html += `<br/><span style="color:#3b82f6">${d.count}</span> occurrences`;
                return html;
            }
            return '';
        }
 ...
 */
function getBlindSpotsTooltipConfig(selectedTopic) {
    return {
        show: true,
        trigger: 'item',
        backgroundColor: 'rgba(15, 23, 42, 0.95)',
 ...
 */
function renderGraph(json, yearLabel) {
    setLoading(false);
    currentGraphData = json;
    const nodeCountEl = document.getElementById('node-count');
    const edgeCountEl = document.getElementById('edge-count');
    if (nodeCountEl)
        nodeCountEl.textContent = json.nodes.length.toString();
    if (edgeCountEl)
        edgeCountEl.textContent = json.edges.length.toString();
 ...
 */
function renderCooccurrenceGraph(json, yearLabel) {
    const sizes = json.nodes.map((n) => n.size || 10);
 ...
 */
function renderSemanticGraph(json) {
    const option = {
        tooltip: getTooltipConfig(),
        animationDurationUpdate: 1500,
        series: [{
 ...
 */
function renderOntologyForce(json) {
    const option = {
        tooltip: getTooltipConfig(),
        animationDurationUpdate: 1500,
        series: [{
 ...
 */
function renderOntologyCircular(json) {
    const option = {
        tooltip: getTooltipConfig(),
        animationDurationUpdate: 1500,
        series: [{
 ...
 */
function renderBlindSpotsForce(json) {
    const topics = json.nodes.filter((n) => n.type === 'topic');
    const concepts = json.nodes.filter((n) => n.type === 'bridging_concept' || n.type === 'blind_spot_concept');
 ...
 */
function renderBlindSpotsBipartite(json) {
    const topics = json.nodes.filter((n) => n.type === 'topic');
    const concepts = json.nodes.filter((n) => n.type === 'bridging_concept' || n.type === 'blind_spot_concept');
    ...
    const topicColorMap = new Map(topics.map((t, i) => [t.id, topicColors[i % topicColors.length]]));
    const option = {
        tooltip: getBlindSpotsTooltipConfig(currentTopic),
        animationDurationUpdate: 1500,
        series: [{
 ...
 */
function renderForceGraph(json, label) {
    const option = {
        tooltip: getTooltipConfig(),
        animationDurationUpdate: 1500,
        series: [{
...
async function loadSelectedYear() {
    ...
    try {
        ...
        if (yearDisplay)
            yearDisplay.textContent = yearLabel;
    }
    catch (error) {
        setLoading(false);
 ...
 */
function applyAllFilters(data, graphType) {
...
function startPlay() {
    ...
    nextYearData();
    playInterval = setInterval(nextYearData, PLAY_INTERVAL_MS);
}
/**
 * Stop slideshow
 */
 ...
 * Load graph by type
 */
async function loadGraphByType(graphType) {
    const config = GRAPH_TYPES[graphType];
    if (!config)
        return;
 ...
 * Switch graph type
 */
async function switchGraphType(graphType) {
    stopPlay();
 ...
 */
async function handleRelationChange(relation) {
 ...
 */
function updateLayoutToggleStyle(isForce) {
 ...
 */
function updateLayoutToggleLabel(graphType) {
 ...
 */
function updateOntologyFilterStyle(isOverlap) {
 ...
 */
function handleOntologyOverlap() {
    ontologyFilter = 'overlap';
    updateOntologyFilterStyle(true);
    if (originalGraphData) {
        const filteredData = applyAllFilters(originalGraphData, currentGraphType);
 ...
 */
function updateGapsToggleStyle(showGapsActive) {
 ...
 */
function updateBestToggleStyle(isBest) {
 ...
 */
function handleYearSliderChange(index) {
